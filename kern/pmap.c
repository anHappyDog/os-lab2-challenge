...
void page_init(void) {
	...
	//challenge 
	//set context register the pgdir address
	asm ("mtc0 %0,$4"::"r"(FASTTLB_PGDIR_ADDRESS):"memory");
	//WR_CP0(FASTTLB_PGDIR_ADDRESS,CP0_CONTEXT);	
}
...
// challenge
int map_pgdir_tokseg2(Pde* apgdir,u_int asid) {
	int pdeno;
	struct Page* pp;
	Pte* pte1;
	for (pdeno = 0; pdeno <= PDX(~0); ++pdeno) {
		if (apgdir[pdeno] & PTE_V) {
			pp = page_lookup(apgdir,KADDR(PTE_ADDR(apgdir[pdeno])),&pte1);
			try(page_insert(apgdir,asid,pp,FASTTLB_PGDIR_ADDRESS + BY2PG * pdeno,PTE_D));
		}
	}
	return 0;
}
